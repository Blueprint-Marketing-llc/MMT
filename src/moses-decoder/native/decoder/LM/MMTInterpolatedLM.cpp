...
// friend
    ...
    MMTInterpolatedLM::MMTInterpolatedLM(size_t startInd, const std::string &line)
            : StatefulFeatureFunction(startInd, line), m_nGramOrder(0), m_enableOOVFeature(false), m_factorType(0),
              logger("decoder.MMTInterpolatedLM") {
        ReadParameters();
    ...
    FFState *MMTInterpolatedLM::BlankState(MemPool &pool, const System &sys) const {
        ...
        auto *s = new(pool.Allocate<ILMState>()) ILMState(
                m_lm->MakeHistoryKey(phrase, (HistoryKey *) pool.Allocate(m_lm->GetHistoryKeySize())));
    ...
    MMTInterpolatedLM::CalcScore(const Phrase<Moses2::Word> &phrase, float &fullScore, float &ngramScore,
        ...
        if (logger.IsTraceEnabled()) {
            LogTrace(logger) << "void MMTInterpolatedLM::CalcScore(const Phrase &phrase, ...) const START phrase:|"
                             << phrase.Debug(system) << "|";
        }
        ...
        if (logger.IsTraceEnabled()) {
            if (context_vec == nullptr) {
                LogTrace(logger)
                        << "void MMTInterpolatedLM::CalcScore(const Phrase &phrase, ...) const context is null";
            } else if (context_vec->empty()) {
                LogTrace(logger)
                        << "void MMTInterpolatedLM::CalcScore(const Phrase &phrase, ...) const context is empty";
            } else {
                LogTrace(logger)
        ...
        for (size_t position = 0; position < phrase_vec.size(); ++position) {
            ...
            double prob = lm->ComputeProbability(phrase_vec.at(position), cursorHistoryKey, context_vec,
                                                 &outHistoryKey);
    ...
    MMTInterpolatedLM::EvaluateInIsolation(MemPool &pool, const System &system,
        ...
        if (logger.IsTraceEnabled()) {
            LogTrace(logger)
                    << "void LanguageModel::EvaluateInIsolation(const Phrase &source, const TargetPhrase &targetPhrase, ...)";
            LogTrace(logger) << "targetPhrase:|" << targetPhrase.Debug(system) << "|";
            LogTrace(logger) << "pthread_self():" << pthread_self();
        }
        ...
        if (logger.IsTraceEnabled()) {
            LogTrace(logger) << "CalcScore of targetPhrase:|" << targetPhrase.Debug(system) << "|: ngr=" << nGramScore
                             << " est=" << estimateScore;
        }
    }
    ...
    void MMTInterpolatedLM::EvaluateWhenApplied(const ManagerBase &mgr,
        ...
        if (logger.IsTraceEnabled()) {
            if (context_vec == nullptr) {
                LogTrace(logger) <<
                                 "void MMTInterpolatedLM::EvaluateWhenApplied(const Phrase &phrase, ...) const context is null";
            } else if (context_vec->empty()) {
                LogTrace(logger) <<
        ...
        if (hypo.GetBitmap().IsComplete()) {
            ...
            score += lm->ComputeProbability(kVocabularyEndSymbol, tmpHistoryKey, t_context_vec.get(),
                                            &cursorHistoryKey);
        } else {
    ...
    void MMTInterpolatedLM::SetParameter(const std::string &key, const std::string &value) {
        if (key == "factor") {
            ...
            LogTrace(logger) << "lm_options.adaptivity_ratio:" << lm_options.adaptivity_ratio;
        } else {
